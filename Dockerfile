...
# 添加和更新仓库
RUN zypper addrepo --check --refresh --priority 90 'http://ftp.uni-erlangen.de/pub/mirrors/packman/suse/openSUSE_Tumbleweed/Essentials/' packman-essentials
RUN zypper --gpg-auto-import-keys refresh || true

# 安装基础包
RUN zypper --gpg-auto-import-keys install --no-confirm \
    python311 python311-pip python311-wheel python311-setuptools \
    python311-devel python311-Cython gcc-c++ python311-py-build-cmake \
    shadow git aria2
...
# 分步安装特定的包
RUN zypper --gpg-auto-import-keys install --no-confirm \
    python311-numpy python311-opencv || true

RUN zypper --gpg-auto-import-keys install --no-confirm \
    python311-ffmpeg-python ffmpeg x264 x265 || true

RUN zypper --gpg-auto-import-keys install --no-confirm \
    python311-dbm || true

RUN zypper --gpg-auto-import-keys install --no-confirm \
    google-noto-sans-fonts google-noto-sans-cjk-fonts google-noto-coloremoji-fonts || true

RUN zypper --gpg-auto-import-keys install --no-confirm \
    Mesa-libGL1 libgthread-2_0-0 || true

# 清理无用文件
RUN rm /usr/lib64/python3.11/EXTERNALLY-MANAGED
